...
def GetUserdata(name) :
    ...
    userdata=json.loads(users_schema.dumps(User.query.filter_by(Name=name)).data)
...
def checkface() :
    ...
    face, flag = DetectFace(data_uri_to_cv2_img(image))
...
def login():
    ...
    if (len(userdata)==0):
        ...
        permission=computeDistance(userSavedEmbedding, capturedEmbedding)
        if(permission):
